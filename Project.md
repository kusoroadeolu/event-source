...
Replay mechanism that reconstructs aggregates from their event stream
...
Query by aggregate ID, by event type, by time range
...
Projections:
Simple projection API - "subscribe to these event types and build a read model"
...
Better event naming - derive from method + maybe a counter for the same method